...
//		size = file.tellg();
//		file.seekg(0, std::ios::beg);
//		int total_node_num, final_node_num, depth_, full_layer_;
//		
//		//node_num_data, node_num_accu_data, key_data, children_data, data_data, label_data;
//		file.read((char*)(&total_node_num), sizeof(int));
//		file.read((char*)(&final_node_num), sizeof(int));
//		file.read((char*)(&depth_), sizeof(int));
//		file.read((char*)(&full_layer_), sizeof(int));
//		
//		char node_num_data[sizeof(int) * 7], node_num_accu_data[sizeof(int) * 8];
//		std::vector <char> key_data;
//
//		file.read(&node_num_data[0], sizeof(int) * 7);
//		file.read(&node_num_accu_data[0], sizeof(int) * 8);
//		file.read((char*)key_data.data(), sizeof(int) * total_node_num);
//		file.read((char*)children_data.data(), sizeof(int) * total_node_num);
//		file.read((char*)data_data.data(), sizeof(int) * 3 * final_node_num);
//		file.read((char*)label_data.data(), sizeof(int) * 3 * final_node_num);
//
//
//		std::string buffer(size, ' ');
//		file.seekg(0, std::ios::beg);
...
void convert_dataset(const string& root_folder, const string& list_file,
	...
	while (std::getline(infile, line)){
		...
		label = atoi(line.substr(pos + 1).c_str()); //atoi ascii to integer
...
int main(int argc, char** argv) {
	...
	convert_dataset("H:/deeplearning/O-CNN/data/", "H:/deeplearning/O-CNN/data/7_tmp.txt", "H:/deeplearning/O-CNN/data/7_train");
	system("pause");
	return 0;
}